...
function pdf_create(
    ...
    $CI->load->library('ZugferdGenerator'); //zugpferd20 test
    $CI = &get_instance();
    ...
    // Zugpferd2.0 test
    if ($zugferd_invoice && isset($CI->invoice) && isset($CI->items)) {
        $xml = $CI->ZugferdGenerator->generate($CI->invoice, $CI->items);
        $mpdf->PDFA = true;
        $mpdf->PDFAauto = true;
        $mpdf->SetAttachXML($xml, 'zugferd-invoice.xml');
    }
